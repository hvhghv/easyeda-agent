...
func AllActions() []ActionSpec {
	return []ActionSpec{
		...
		{
			Name:        "system.transaction.release",
			Domain:      DomainSystem,
			Phase:       1,
			NeedsWindow: true,
			Description: "Release the calling CLI command's per-window transaction lease. Daemon-local; normal CLI users never need to call it directly.",
			Outputs:     []string{"released", "windowId"},
		},
		{
			Name:        "system.notify",
			Domain:      DomainSystem,
		...
		// ── native library authoring (symbol / footprint / device) ──────
		// These actions deliberately use the official library editors and
		// primitive APIs. They are not an import path: every pin, pad, and
		// graphic is created natively and the resulting library document is
		// saved before it can be associated with a device.
		{
			Name:        "library.info",
			...
			Mutates:     false,
			NeedsWindow: true,
			Description: "Read the active workspace's system, personal, project, favorite, and registered external library UUIDs. Use this before native library authoring, especially in offline/half-offline desktop mode where some library classes may be unavailable or read-only.",
			Outputs:     []string{"systemLibraryUuid", "personalLibraryUuid", "projectLibraryUuid", "favoriteLibraryUuid", "libraries[]"},
		},
		{
			...
			Phase:       1,
			NeedsWindow: true,
			Description: "Read one native symbol by UUID and library UUID without mutation.",
		...
		},
		{
			...
			Phase:       1,
			NeedsWindow: true,
			Description: "Read one native footprint by UUID and library UUID without mutation.",
			Inputs:      []string{"uuid", "libraryUuid"},
			Outputs:     []string{"item"},
		},
		{
			...
			Phase:       1,
			NeedsWindow: true,
			Description: "Read one native device by UUID and library UUID without mutation.",
			Inputs:      []string{"uuid", "libraryUuid"},
			Outputs:     []string{"item"},
		},
		{
			...
			Mutates:     true,
			NeedsWindow: true,
			Description: "Create a native EasyEDA symbol in a writable library, open its symbol editor, draw pins and graphics with typed primitive APIs, save, verify counts, close the library tab, and restore the previously active document. Defaults to the personal library (project library fallback). This is native authoring, not import.",
			Inputs: []string{
				"name", "libraryUuid optional", "target optional (personal|project, default personal)",
				"description optional", "pins[] {number,name,x,y,rotation,length,type,shape}",
				"rectangles[] optional", "circles[] optional", "polygons[] optional", "texts[] optional",
			},
			Outputs:    []string{"libraryUuid", "symbolUuid", "name", "saved", "counts{pins,rectangles,circles,polygons,texts}", "verified"},
			VerifyWith: []string{"schematic.library.search"},
		},
		{
			...
			Mutates:     true,
			NeedsWindow: true,
			Description: "Create a native EasyEDA footprint in a writable library, open its footprint editor, draw pads and line artwork with typed primitive APIs, save, verify counts, close the library tab, and restore the previously active document. Defaults to the personal library (project library fallback). This is native authoring, not import.",
			Inputs: []string{
				"name", "libraryUuid optional", "target optional (personal|project, default personal)",
				"description optional", "pads[] {number,x,y,layer,rotation,shape,width,height,cornerRadius,hole,metallization,padType}",
				"lines[] optional {layer,startX,startY,endX,endY,lineWidth}",
			},
			Outputs:    []string{"libraryUuid", "footprintUuid", "name", "saved", "counts{pads,lines}", "verified"},
			VerifyWith: []string{"schematic.library.search"},
		},
		{
			...
			Mutates:     true,
			NeedsWindow: true,
			Description: "Create a native EasyEDA device by associating an existing native symbol and optional footprint, then read it back and verify the association. Defaults to the personal library (project library fallback).",
			Inputs: []string{
				"name", "libraryUuid optional", "target optional (personal|project, default personal)",
				"symbol {uuid,libraryUuid}", "footprint optional {uuid,libraryUuid}", "description optional",
				"designator optional", "addIntoBom optional", "addIntoPcb optional", "manufacturer/manufacturerId/supplier/supplierId optional", "otherProperty optional",
			},
			Outputs:    []string{"libraryUuid", "deviceUuid", "name", "device", "verified"},
			VerifyWith: []string{"schematic.library.search"},
		},
		...
		// set-paper-size (A4/A3) API; the title block is the editable "图纸" surface.
		{
			...
			Phase:       1,
			NeedsWindow: true,
			Description: "Read a schematic page's 明细表 (title block): show flag + per-field data. Defaults to the focused page; pass pageUuid for another. This is a read-only projection, not a write payload.",
			Inputs:      []string{"pageUuid optional"},
			Outputs:     []string{"showTitleBlock", "titleBlockData (field keys + values)"},
		},
		{
			Name:        "schematic.titleblock.health",
			Domain:      DomainSchematic,
			Phase:       1,
			NeedsWindow: true,
			Description: "Read-only health probe for the active page title-block model: require non-empty core fields, a valid sheet primitive/bbox, and an official DRC with no fatal/error findings. Run sch titleblock-health --reload to verify persistence across a document reload.",
			Outputs:     []string{"healthy", "pageUuid", "requiredKeysPresent", "missingKeys", "fieldCount", "sheetBBox", "drcSummary", "diagnostics"},
		},
		{
			...
			Phase:       1,
			NeedsWindow: true,
			Description: "Retired safety boundary. Always returns UNSUPPORTED_RISKY_OPERATION and never calls EasyEDA's title-block write API, which can corrupt the internal sheet model.",
			Outputs:     []string{"UNSUPPORTED_RISKY_OPERATION"},
		},
		{
		...
		{
			Name:        "schematic.netlabel.create",
			Domain:      DomainSchematic,
			Phase:       1,
			Mutates:     true,
			NeedsWindow: true,
			Description: "Create a normal N-shaped schematic net label. EasyEDA 3.2 uses the editor's native toolbar/canvas interaction because its public createNetLabel stub never settles; the result is verified from the rendered name plus native netlabel-cross. Newer editors use the official API. This never substitutes a net port or plain text.",
			Inputs:      []string{"net", "x", "y"},
			Outputs:     []string{"primitive id", "net", "coordinates", "backend", "verified"},
		...
		{
			Name:        "schematic.wires.list",
			Domain:      DomainSchematic,
			Phase:       1,
			Mutates:     false,
			NeedsWindow: true,
			Description: "Read-only list of wire primitives on the ACTIVE schematic page, preserving each owning primitive id and complete polyline for typed rigid-group moves.",
			Inputs:      []string{},
			Outputs:     []string{"count", "scope (activePage)", "wires[].primitiveId", "wires[].line", "wires[].net"},
		},
		{
			Name:        "schematic.graphics.create",
			Domain:      DomainSchematic,
			Phase:       1,
			Mutates:     true,
			NeedsWindow: true,
			Description: "Create schematic annotation rectangles and text with typed primitive APIs, verify every returned primitive id, and save the active page.",
			Inputs:      []string{"rectangles[] optional", "texts[] optional"},
			Outputs:     []string{"rectangles[] primitive ids", "texts[] primitive ids", "saved", "verified"},
			VerifyWith:  []string{"schematic.text.list", "schematic.check"},
		},
		{
			Name:        "schematic.graphics.delete",
			Domain:      DomainSchematic,
			Phase:       1,
			Mutates:     true,
			NeedsWindow: true,
			Description: "Delete recorded schematic annotation rectangle/text ids through the generic primitive API, verify no id survives, and save the active page.",
			Inputs:      []string{"ids[]"},
			Outputs:     []string{"requested", "deleted", "survived", "saved", "verified"},
			VerifyWith:  []string{"schematic.text.list", "schematic.check"},
		},
		{
		...
		},
		{
			...
			Inputs:      []string{"pinX", "pinY", "kind", "net", "direction optional", "offset optional", "rotation optional", "rotationNegates optional calibrated boolean"},
		...
		},
		{
			...
			Phase:       1,
			NeedsWindow: true,
			Description: "ONE-call semantic snapshot of the whole circuit — so the agent reads everything at once instead of stitching components.list + netlist + check. Returns: components[] (primitiveId — the MUTATION handle for select/modify/delete/replace/rebind — plus designator, type, name/value, uniqueId, footprint, supplierId=LCSC, x/y, pins[] each with its JSON-authoritative net), nets[] (net → connected {designator.pin} keys, degree, isGlobal power/ground flag), floatingPins[] (only when the manufacture netlist is available), and the geometric design check (same as schematic.check; pass includeCheck=false to skip for a faster read). netlistAvailable/netlistError make failure explicit: on unavailable netlists, pin net=null means unknown and is tagged netState=unavailable — it is never reported as a floating pin. WARNING: uniqueId (\"gge…\") is the sch↔PCB link key, NOT a primitiveId — feeding it to by-id mutations returns notFound (live incident: primitiveId used to be missing from this output, so agents grabbed uniqueId and every mutation failed).",
			Inputs:      []string{"allPages optional", "includeCheck optional (default true)"},
			Outputs:     []string{"netlistAvailable", "netlistError when unavailable", "components[].primitiveId (mutation handle)", "components[]", "componentCount", "nets[]", "netCount", "floatingPins[]", "floatingPinCount", "check"},
		},
		{
			Name:        "schematic.save",
			Domain:      DomainSchematic,
		...
		{
			Name:        "pcb.export.manufacture",
			Domain:      DomainPcb,
			Phase:       2,
			NeedsWindow: true,
			Description: "Export EasyEDA's complete PCB manufacturing-data package through pcb_ManufactureData.getManufactureData. Read-only; returns a persisted ZIP artifact.",
		...
		},
		{
			...
			Phase:       2,
			NeedsWindow: true,
			Description: "Export Gerber fabrication data through pcb_ManufactureData.getGerberFile. Read-only; returns a persisted ZIP artifact.",
			Inputs:      []string{"fileName optional", "colorSilkscreen optional"},
			Outputs:     []string{"artifact id", "file path", "file name", "size"},
			VerifyWith:  []string{"pcb.drc.check", "pcb.report"},
		},
		{
			Name:        "pcb.export.pick_place",
			Domain:      DomainPcb,
			Phase:       2,
			NeedsWindow: true,
			Description: "Export pick-and-place coordinates through pcb_ManufactureData.getPickAndPlaceFile. Read-only; CSV or XLSX.",
			Inputs:      []string{"fileName optional", "fileType optional (csv|xlsx, default csv)"},
			Outputs:     []string{"artifact id", "file path", "file name", "size"},
			VerifyWith:  []string{"pcb.components.list"},
		},
		{
			Name:        "pcb.export.ipc2581",
			Domain:      DomainPcb,
			Phase:       2,
			NeedsWindow: true,
			Description: "Export IPC-2581C manufacturing data through pcb_ManufactureData.getIpc2581CFile. Read-only; XML, CVG, or 2581.",
			Inputs:      []string{"fileName optional", "fileType optional (xml|cvg|2581)", "oemNumber optional (Device|Manufacturer Part|Supplier Part|Comment)"},
			Outputs:     []string{"artifact id", "file path", "file name", "size"},
			VerifyWith:  []string{"pcb.drc.check", "pcb.report"},
		},
		{
