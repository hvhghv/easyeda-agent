...
> | `adopt ✓ …确实没有落地(回读已证新鲜)` | 可证的最新一刻那里没有新器件 | 无 —— 可以直接重跑 |
> | `adopt ? …无法判断` | **回读不可信**,落没落地都有可能 | 照处方去页面上查一眼,别盲重跑(会造重复件) |
> | `adopt ✗ …` | 没快照 / 回读失败 | 同上 |
>
> 边界:**第一件(anchor)就超时时没有任何探针可用**,只能报 uncertain —— 那一刻「没落地」
> 与「读得太早」在观测上完全等价(页面本就该等于落地前快照),没有任何机械判据能分开,
> 所以不假装能证明。反过来,删除侧的「证实已删」不需要这道门:读得太早只会把已删的报成
> `survived`(偏保守,自带重删+处方),不会造成假安全。
>
> ⚠️ **残件删不掉 ≠ 未提交 ≠ 回读 stale**:同一份失败报文里**有回执的**器件也会
> `survived`——那是**连接器 action 队列 wedge**(某个重调用的 promise 永不 resolve,此后
> `place`/`delete`/`document.open` 整体被吞,而轻读照常)。处方固定:`sch save` → **完全退出
> 并重启 EasyEDA** → 再删。反复 `prim-delete` / `doc reload` 在 wedge 期一律无效。
>
...
> **「不得变差」门:收敛使本区排不下时保留原形(2026-08-20 真机取证)。**
> phase A 对**大符号单件组**曾是负优化 —— 真机 `MCU_IO` 的
> `esp32s3_wroom1_module`(单件 ESP32-S3-WROOM-1,41 脚,本体 71×421,标签横向
> 铺开)`433×541 → 244×767`:宽收了 189,**高涨了 226**,而可用高只有 765
> ——「不收敛能排,收敛了排不下」。现在 phase A 逐区加一道门:
>
> - 判据是**可排布性**,不是面积/周长 —— 上例宽度和面积都变小了,照样是负优化。
>   门问的是「这个尺寸在 A4 域界 + 图签 keep-out 下**还存不存在落点**吗」
>   (图签左侧整条 396 宽 / 上方整条 555 高,与 phase B 的 `纸面放不下` 同一把尺);
> - **原形可排 ∧ 收敛后不可排 → 保留原形**(不重排、不重生桩,刚体平移到落位框);
>   原形本来就排不下时不拦(收敛是唯一出路);两维都没变大时结构上不会变差;
> - **逐区独立**:一个区回退不影响其它区照常收敛;
> - **绝不静默**:人读输出该区行首是 `↩`,`zones[].mode` 尾巴与 JSON 的
>   `zones[].retained` / `zones[].retainWhy` 都带这句 ——
>   `收敛回退:高 541→767 后本页无落点(可用 1110×765;图签上方高 555、左侧宽 396)—— 保留原形 433×541`。
>
