...
- `schematic.netlabel.create` — 创建普通 N 形网络标签。嘉立创 EDA 当前正式客户端 3.2 的公开 `createNetLabel` 为会挂起的存根，因此连接器改用**编辑器自身**的 `placeNetLabel` 放置控制器：缩放到目标原理图坐标后仅向当前画布派发 DOM 指针移动事件，由编辑器给出网格吸附点与候选导线；只有命中已有导线时才提交名称，并从内部模型读回 `netlabel`、网名和父导线。绝不降级成箭头网络端口、普通文本，也不操作系统鼠标或键盘。CLI：`easyeda sch netlabel --net FB_3V3 --x 215 --y 390`
