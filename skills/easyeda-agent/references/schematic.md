...
两档),用户/agent 的直觉操作反而破坏收敛成果。**恢复段有意不夹上限**:那是火警
现场,把连接接回来优先于把框收窄;它带来的伸展由 `zone-arrange --apply` 的断言③
落地复判如实报出来。
...
> (本体 ∪ 网名带)`,所以**规划框是落地框的可靠预测**——更准确说是**上界**:框里
> 已含一格落地余量(桩端点的 5 网格吸附),人读/JSON 的 `zones[].mode` 尾巴上写着
> `· 落地余量 5(…规划框=落地框上界)`,所以复判只判「落地比规划**胖**」。三条推论:
>
...
与规划框逐区比。输出形如 `复判 U:实测框 353×382 / 规划框 315×351`。偏差 > `--gutter`
或实测区框互相重叠 → **如实报红并以非零退出**(电气与位姿仍已落地保存,不回滚)。
