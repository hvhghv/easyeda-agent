...
- `schematic.primitives.delete` — 按 id **跨类型**删除(组件/标志/导线/总线/图形都行),省略 `--ids` 则删当前选区(配合 `schematic.select` 做"全选→删除")。无 undo,确认门控。CLI:`easyeda sch prim-delete [--ids '[...]']`。**计数是回读验证出来的,不是请求数(#164)**:删完重新枚举各类目,`deleted`/`total` 只计真正消失的;有图元活下来则 `result.partial:true` + `survived`(按类目列 id),CLI **非零退出**。此前它把请求数当删除数上报,于是「删旧+重画」的 zone-draw 标签每轮都报干净、实则只加不减(P5 累积到 56 个)。**平台还有更深一层坑**:某些图元 delete 后立即读=0、`sch save` 后读=0,但 `doc reload` 后原 id 全部复活 —— 立即回读证明不了持久化,清理这类残留后请 `doc reload` 再 `sch text-list` 复查,仍在就去 EasyEDA UI 里删。
