...
> 它随后走和正常件一样的逐个删+回读证实。命中 0 个 = **证实这次 place 没落地**(不再吓唬
> 你"可能建了个残件"),**绝不凭空造 id**;页面上原有的同型同坐标器件天生在快照里,永远
> 不会被收编或误删;快照读不到就**整个关掉收编**并如实报 PARTIAL STATE。命中 ≥2 个则不
> 收编但**逐一点名**,并打印可直接跑的 `sch prim-delete --ids …`。
>
