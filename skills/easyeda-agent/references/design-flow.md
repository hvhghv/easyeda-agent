...
- **分页 + 分区框 + 电路说明,三件套齐了才算原理图组织完成**(默认必做,不是装饰 ——
  ...
  - **这三件现在是机械判据,不再靠自觉**:`sch check` 出
    ...
    **归属判据 `note-outside-zone`**(放对没有):每条 `--zone` 登记的说明,其渲染
    ...
    (手填必然飘)。`--zone` 的名字**全名/末段短名/组 id/唯一前缀都命中**(与
