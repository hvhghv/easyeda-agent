...
- **分页 + 分区框 + 电路说明,三件套齐了才算原理图组织完成**(默认必做,不是装饰 ——
  ...
  - **这三件现在是机械判据,不再靠自觉**:`sch check` 出
    ...
    (手填必然飘,而且挤不下时会掉到框外)。说明只写**需要注意的**(关键参数、易错点),
    一到两行,不要复述电路。
  - **分区粒度要细**:一个块不等于一个区。按功能再拆(如 CH340C 那页拆成
    `USB_C`(Type-C + CC 下拉)/ `ESD`(差分保护)/ `BRIDGE`(桥芯片 + 去耦)),
    整块糊成一个大框等于没分区 —— 框会摊到大半张纸、里面大片空白,读图的人得不到任何信息。
