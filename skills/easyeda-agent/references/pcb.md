...
1. `easyeda daemon health` → confirm a connected window (route by `--project <name>`; `--window <windowId>` only for fine control). Context is live — refreshed on every action AND, with connector ≥ v0.5.7, pushed by the heartbeat within ~3s of a UI tab-switch (so health follows the UI even with no command run). `connectorVersionOk: false` flags a stale connector loaded in an open window (fully quit + relaunch EasyEDA).
   **同一 window 的命令全程串行**:一个顶层 CLI 调用现在持有 command transaction,
   防止别的进程插进 `--doc` guard 与实际 action 之间;不要用并行工具同时驱动同一窗口。
   只有不访问 EDA 的本地源文件/图片/JSON 分析可以并行。
...
- **多页工程必须绑定父 schematic,不能绑定当前 page**。`sch pages` 同时区分 page UUID
  与 parent schematic UUID;`import-changes` 前用 `board current`/`pcb board-info` 回读,
  确认 Board.schematic 指向父对象。若绑定子页或旧 UUID,先 `board rebind`,否则会出现
  Netlist Error,即使逐页连接本身完全正确。
...
- `easyeda pcb layout-lint` — **pre-route only** placement/routability predictor. It uses the
  real board-outline polygon, not only its axis-aligned bbox. `off-board` means pad extent/probe
  outside and is always blocking; `body-outside` means component body/rendered bbox overhang and
  is separately reported as WARN, or blocking with `--fail-body-outside`. An edge connector may
  overhang only when S0 declares its allowed direction/distance; RF modules and ordinary parts use
  strict body containment. Courtyard has no independent typed gate yet, so inspect footprint data
  plus the final snapshot. Once routing exists, do not cite ratsnest/crossing score as finished-board
  quality; use DRC, unrouted count, `pcb report`, and `pcb check`.
...
### Manufacturing exports (fresh-gate only)

Typed official-API exports are available; never export directly from a dirty canvas:

```powershell
easyeda pcb save
easyeda doc reload PCB1
easyeda pcb drc --json --timeout 120
easyeda pcb check --json
easyeda pcb export-manufacture --out release/manufacture.zip
easyeda pcb export-gerber --out release/gerber.zip
easyeda pcb export-pick-place --type csv --out release/pick-place.csv
easyeda pcb export-ipc2581 --type 2581 --out release/design.2581
Get-FileHash release/* -Algorithm SHA256
```

Hard gate: native DRC `total=0` and DFM `summary.errors=0`. `pcb check` may still report
`passed:false` because WARN exists; retain every warning in the delivery report instead of calling
it a manufacturing error or silently discarding it. After any mutation, restart at save/reload.
The four export commands map to official `getManufactureData`, `getGerberFile`,
`getPickAndPlaceFile`, and `getIpc2581CFile` and persist the returned artifact through the daemon.
...
- Do not run two window-touching commands concurrently against the same EasyEDA window. The daemon
  transaction lease is a safety net for separate processes, not permission to create contention.
- For enclosure approval, report pad outside and body overhang separately. A zero pad-outside count
  does not prove an ESP/RF module body or antenna section is inside the real outline.
- Treat `layout-lint` as a pre-route tool. Post-route release uses DRC/unrouted/report/DFM plus a
  visual snapshot review; no one signal can replace the others.
